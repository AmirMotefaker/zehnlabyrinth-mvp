...
const ENERGY := Color("5de9e2")
const ENERGY_GLOW := Color(0.36, 0.92, 0.89, 0.16)
const NODE_IDLE := Color("58e3dd")
const GOAL_IDLE := Color("9278ff")
const GOAL_ON := Color("ffe582")
...
func _board_style() -> StyleBoxFlat:
	...
	style.bg_color = Color("121b41")
	style.border_color = Color("43568f")
...
func _draw_beams(segments: Array) -> void:
	for segment in segments:
		...
		draw_line(start, finish, ENERGY_GLOW, 28.0, true)
		draw_line(start, finish, ENERGY, 9.0, true)
...
func _draw_blockers() -> void:
	for blocker in session.level.blockers:
		...
		draw_rect(rect, Color("252a44"), true)
		draw_rect(rect, Color("66729b"), false, 4.0)
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(0, rect.size.y * 0.65), "×", HORIZONTAL_ALIGNMENT_CENTER, int(rect.size.x), int(rect.size.y * 0.58), Color("a9b7ea"))
...
func _draw_relays(result: Dictionary) -> void:
	for relay_index in range(session.level.relays.size()):
		...
		draw_circle(point, radius, Color("6a5728") if lit else Color("2d2437"))
		draw_arc(point, radius, 0, TAU, 36, GOAL_ON if lit else Color("ffb55c"), 5.0, true)
		draw_string(ThemeDB.fallback_font, point + Vector2(-radius, radius * 0.34), str(relay_index + 1), HORIZONTAL_ALIGNMENT_CENTER, int(radius * 2), int(radius * 0.8), Color.WHITE)
...
func _draw_pieces(result: Dictionary) -> void:
	for index in range(session.level.pieces.size()):
		...
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(0, rect.size.y * 0.66), glyph, HORIZONTAL_ALIGNMENT_CENTER, int(rect.size.x), int(rect.size.y * 0.62), Color.WHITE)

func _glyph(piece: Dictionary, orientation: int) -> String:
	if piece.kind == "rail": return "—" if orientation == 0 else "│"
...
func _draw_node(cell: Vector2i, text: String, color: Color, lit: bool) -> void:
	...
	if lit: draw_circle(point, radius + 13.0, Color(color, 0.17))
	draw_circle(point, radius, Color("14234c") if not lit else Color("65572d"))
	draw_arc(point, radius, 0, TAU, 48, color, 6.0, true)
	draw_string(ThemeDB.fallback_font, point + Vector2(-radius, radius * 0.28), text, HORIZONTAL_ALIGNMENT_CENTER, int(radius * 2), int(radius * 0.62), Color.WHITE if lit else color)

func _piece_index(cell: Vector2i) -> int:
	for index in range(session.level.pieces.size()):
		if session.level.pieces[index].cell == cell: return index
	return -1
